...
RESULTS_PATH = "/CODE/PARAM_SEARCH_AUTHENTICATION_OVR_EMG/"
#################################################################################################################
#
# load the data
#
...
for run in range(1,5):
    ...
    report_filename = RESULTS_PATH+"auth_ovr_ch1_"+str(num_ch_1)+"_k1_"+str(kernel_1)+"_run_"+ str(run) + ".csv"
    for winSize in [100,1000, 10000,25000]:
        gc.enable()
        for ovr in np.arange(0,1,0.1):
            ...
            for ifold in range(nfold):
                start = time.time()
                print('running fold = %d for window size = %d\nnumber of channels layer 1 = %d\nkernel layer 1 = %d \nwindow = %d\noverlap = %d' % (
                ifold, winSize, num_ch_1, kernel_1, winSize, ovr*100))
