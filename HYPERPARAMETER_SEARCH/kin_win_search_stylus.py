# running models with kinematics data using the stratified cv approach
import os.path
import random
import numpy as np
import pandas as pd
...
def create_model(X_train, y_train,  kernel_1, num_ch_1, dropout, EPOCHS, BATCH_SIZE, lr, LOSS):
    ...
    # tcnn_block_2 = Conv1D(filters=num_ch_2, kernel_size=kernel_2, padding='valid', name="tcnn_block_2")(drop_1)
    # activation_2 = Activation("relu")(tcnn_block_2)
    # bn_2 = BatchNormalization()(activation_2)
    # drop_2 = Dropout(dropout)(bn_2)
    self_att_1 = Attention(32, "CustomLayer")(drop_1)
    flat_1 = Flatten(name='Flatten')(self_att_1)
    # dense_1 = Dense(256, activation='relu', name='dense_1')(flat_1)
    # bn_3 = BatchNormalization()(dense_1)
    # drop_3 = Dropout(dropout)(bn_3)
    # dense_2 = Dense(128, activation='relu', name='dense_2')(drop_3)
    # bn_4 = BatchNormalization()(dense_2)
    # drop_4 = Dropout(dropout)(bn_4)
    # dense_3 = Dense(64, activation='relu', name='dense_3')(drop_4)
    # bn_5 = BatchNormalization()(dense_3)
    # drop_5 = Dropout(dropout)(bn_5)
    # dense_4 = Dense(32, activation='relu', name='dense_4')(drop_5)
    # bn_6 = BatchNormalization()(dense_4)
    # drop_6 = Dropout(0.25)(bn_6)
...
RANDOM_STATE_PER_RUN = [42,9,23,101,13] # we select separate random state per each run

RESULTS_PATH = "/scratch/vb1121/CODE/PARAM_SEARCH_AUTHENTICATION_WIN_STYLUS/"
#################################################################################################################
#
# load the data
#
...
for run in [1]:#range(1,5):
    RANDOM_STATE = RANDOM_STATE_PER_RUN[run-1]
    ros = RandomOverSampler(random_state=RANDOM_STATE)

    report_filename = RESULTS_PATH+"authen_win_ch1_"+str(num_ch_1)+"_k1_"+str(kernel_1)+"_run_"+ str(run) + ".csv"
    for winSize in np.arange(128,1774,64):# median 928
        gc.enable()
        ...
        if os.path.isfile(report_filename):
            ...
            reports_df = pd.DataFrame(
                columns=['WINDOW', 'OVERLAP', 'EPOCHS', 'LEARNING_RATE', 'BATCH_SIZE', 'DROPOUT', 'ACC', 'PRECISION',
        ...
        for ifold in range(nfold):
            ...
            reports_df.loc[len(reports_df)] = [winSize, ovr, EPOCHS, lr, BATCH_SIZE, dropout, report["accuracy"],
                                               avg_precision,
