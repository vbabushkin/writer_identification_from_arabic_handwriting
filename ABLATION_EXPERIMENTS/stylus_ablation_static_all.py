# ablation experiments stylus kinematics removing all static features
import gc
import os
import os.path
import pickle
...
num_ch_1 = 2048
kernel_1 = 100
RESULTS_PATH = "/scratch/vb1121/CODE/AUTH_STYLUS/"
MAIN_FILENAME = RESULTS_PATH + "abl_auth_ch1_" + str(num_ch_1) + "_k1_" + str(kernel_1) + "_w_" + str(
    winSize) + "_stylus_static_all_parallelized"
...
indices = {feat: features_7.index(feat) for feat in targets if feat in features_7}
print(indices)
target_indices = [features_7.index(feat) for feat in targets]
