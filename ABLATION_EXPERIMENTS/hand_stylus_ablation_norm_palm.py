# ablation experiments normalizing hand stylus kinematics coordinates wrt palm
import gc
import os
import os.path
import pickle
...
MAIN_FILENAME = RESULTS_PATH + "abl_auth_ch1_" + str(num_ch_1) + "_k1_" + str(kernel_1) + "_w_" + str(
    winSize) + "_hand_stylus_norm_palm_parallelized_200"
...
excluded_keywords = ["speed", "normal", "rotation", "width", "length", "angle", "strength", "distance", "altitude",
                     "azimuth", "pitch", "yaw", "roll"]
hand_x_features = [f for f in features_117 if (f.startswith("x_") or f.endswith("_x")) and f != "x" and not any(
    k in f.lower() for k in excluded_keywords)]
hand_y_features = [f for f in features_117 if (f.startswith("y_") or f.endswith("_y")) and f != "y" and not any(
    k in f.lower() for k in excluded_keywords)]
hand_z_features = [f for f in features_117 if (f.startswith("z_") or f.endswith("_z")) and f != "z" and not any(
    k in f.lower() for k in excluded_keywords)]

hand_x_indices = [features_117.index(f) for f in hand_x_features]
hand_y_indices = [features_117.index(f) for f in hand_y_features]
hand_z_indices = [features_117.index(f) for f in hand_z_features]

palm_x_idx = features_117.index("handCenter_x")
palm_y_idx = features_117.index("handCenter_y")
palm_z_idx = features_117.index("handCenter_z")
...
X_train[:, :, hand_x_indices] = X_train[:, :, hand_x_indices] - X_train[:, :, palm_x_idx:palm_x_idx + 1]
X_train[:, :, hand_y_indices] = X_train[:, :, hand_y_indices] - X_train[:, :, palm_y_idx:palm_y_idx + 1]
X_train[:, :, hand_z_indices] = X_train[:, :, hand_z_indices] - X_train[:, :, palm_z_idx:palm_z_idx + 1]

X_test[:, :, hand_x_indices] = X_test[:, :, hand_x_indices] - X_test[:, :, palm_x_idx:palm_x_idx + 1]
X_test[:, :, hand_y_indices] = X_test[:, :, hand_y_indices] - X_test[:, :, palm_y_idx:palm_y_idx + 1]
X_test[:, :, hand_z_indices] = X_test[:, :, hand_z_indices] - X_test[:, :, palm_z_idx:palm_z_idx + 1]
