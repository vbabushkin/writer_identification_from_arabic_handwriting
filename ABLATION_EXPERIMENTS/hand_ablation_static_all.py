# ablation experiments hand  kinematics removing all static features
import gc
import os
import os.path
import pickle
...
targets=[
    ...
    "handPredictedPinchPosition_x", "handPredictedPinchPosition_y", "handPredictedPinchPosition_z",
...
num_classes = 50
MAIN_FILENAME =RESULTS_PATH+"abl_auth_ch1_" + str(num_ch_1) + "_k1_" + str(kernel_1)+"_w_"+str(winSize)+"_hand_static_all_parallelized_200"
...
indices = {feat: features_110.index(feat) for feat in targets if feat in features_110}
print(indices)
target_indices = [features_110.index(feat) for feat in targets]
...
X_train = np.delete(X_train, target_indices, axis=2)
X_test = np.delete(X_test, target_indices, axis=2)
