# ablation experiments normalizing hand kinematics coordinates wrt palm
import gc
import os
import os.path
import pickle
...
MAIN_FILENAME = RESULTS_PATH + "abl_auth_ch1_" + str(num_ch_1) + "_k1_" + str(kernel_1) + "_w_" + str(
    winSize) + "_hand_norm_palm_parallelized_200"
...
hand_x_features = [f for f in features_110 if (f.startswith("x_") or f.endswith("_x")) and f != "x" and not any(
    k in f.lower() for k in excluded_keywords)]
hand_y_features = [f for f in features_110 if (f.startswith("y_") or f.endswith("_y")) and f != "y" and not any(
    k in f.lower() for k in excluded_keywords)]
hand_z_features = [f for f in features_110 if (f.startswith("z_") or f.endswith("_z")) and f != "z" and not any(
    k in f.lower() for k in excluded_keywords)]
...
palm_x_idx = features_110.index("handCenter_x")
palm_y_idx = features_110.index("handCenter_y")
palm_z_idx = features_110.index("handCenter_z")
