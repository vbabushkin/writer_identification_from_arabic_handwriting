# ablation experiments normalizing hand stylus kinematics coordinates wrt wrist
import gc
import os
import os.path
import pickle
...
num_classes = 50
MAIN_FILENAME = RESULTS_PATH + "abl_auth_ch1_" + str(num_ch_1) + "_k1_" + str(kernel_1) + "_w_" + str(winSize) + "_hand_stylus_norm_wrist_parallelized_200"
...
hand_x_features = [f for f in features_117 if (f.startswith("x_") or f.endswith("_x")) and f != "x" and not any(k in f.lower() for k in excluded_keywords)]
hand_y_features = [f for f in features_117 if (f.startswith("y_") or f.endswith("_y")) and f != "y" and not any(k in f.lower() for k in excluded_keywords)]
hand_z_features = [f for f in features_117 if (f.startswith("z_") or f.endswith("_z")) and f != "z" and not any(k in f.lower() for k in excluded_keywords)]
...
wrist_x_idx = features_117.index("handWristPosition_x")
wrist_y_idx = features_117.index("handWristPosition_y")
wrist_z_idx = features_117.index("handWristPosition_z")
